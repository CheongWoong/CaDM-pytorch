...
class SingleHeadDecoder(nn.Module):
    ...
    def forward(self, x, context):
        if self.training:
            ...
            target_mask = x["future_bool"][:,1:].unsqueeze(-1)

            if context is not None:
                forward_obs = torch.tile(forward_obs[None,:,:,:], [self.ensemble_size, 1, 1, 1])
                backward_obs = torch.tile(backward_obs[None,:,:,:], [self.ensemble_size, 1, 1, 1])
                action = torch.tile(action[None,:,:,:], [self.ensemble_size, 1, 1, 1])
                forward_target = torch.tile(forward_target[None,:,:,:], [self.ensemble_size, 1, 1, 1])
                backward_target = torch.tile(backward_target[None,:,:,:], [self.ensemble_size, 1, 1, 1])
                target_mask = torch.tile(target_mask[None,:,:,:], [self.ensemble_size, 1, 1, 1])
