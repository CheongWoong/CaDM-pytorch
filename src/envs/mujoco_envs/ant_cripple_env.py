    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        ...
        **kwargs
        ...
        #####
        assert self._terminate_when_unhealthy == True
        assert self._use_contact_forces == False
        assert self._exclude_current_positions_from_observation == True
        obs_shape = 27 + 1
        #####
    ...
    def step(self, action):
        self.xy_position_before = self.get_body_com("torso")[:2].copy() #####
        if self.cripple_mask is not None:
            action = self.cripple_mask * action
        self.do_simulation(action, self.frame_skip)
        self.xy_position_after = self.get_body_com("torso")[:2].copy() #####

        xy_velocity = (self.xy_position_after - self.xy_position_before) / self.dt #####
        ...
        info = {
            ...
            "x_position": self.xy_position_after[0], #####
            "y_position": self.xy_position_after[1], #####
            "distance_from_origin": np.linalg.norm(self.xy_position_after, ord=2), #####
    ...
    def _get_obs(self):
        ...
        if self._use_contact_forces:
            ...
            return np.concatenate((self.xy_position_after[:1], position, velocity, contact_force)) #####
        else:
    ...
    def change_env(self):
        self.xy_position_after = self.xy_position_before = self.get_body_com("torso")[:2].copy()

        # Pick which legs to remove
        if self.extreme_set == [0]:
            self.crippled_legs = np.array([self.np_random.choice(self.cripple_set)])
        elif self.extreme_set == [1]:
            self.crippled_legs = self.np_random.choice(self.cripple_set, 2, replace=False)
        else:
    ...
    def get_sim_parameters(self):
        return np.concatenate((self.crippled_legs.reshape(-1), np.array([self.mass_scale])))
    ...
    def reward(self, obs, action, next_obs):
        ...
        costs = ctrl_cost = self._ctrl_cost_weight * torch.sum(torch.square(action), dim=-1)
